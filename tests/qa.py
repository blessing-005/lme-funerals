...
required=['index.html','contact.html','assets/css/styles.css','assets/js/site.js','favicon.svg','favicon.ico','apple-touch-icon.png','icon-192.png','icon-512.png']
...
for f in site.rglob('*.html'):
 ...
 for attr in re.findall(r'(?:src|href)="([^"]+)"',s):
  if attr.startswith(('http:','https:','mailto:','tel:','#','data:','javascript:')): continue
  target=(f.parent/attr.split('?',1)[0].split('#',1)[0]).resolve()
  if attr and not target.exists(): errors.append(f'{f.name}: broken path {attr}')
 lower=s.lower()
 for phrase in ['supplied material','source material','uploaded assets','our research','prototype','mockup','private review','lorem ipsum']:
  if phrase in lower: errors.append(f'{f.name}: internal wording: {phrase}')
